...
import ContratacaoSoftwareForm from "@/components/forms/ContratacaoSoftwareForm";

const Softwares = () => {
  const [formOpen, setFormOpen] = useState(false);
  const [selectedSoftware, setSelectedSoftware] = useState<{nome: string, preco: string} | null>(null);
  const [tipoSolicitacao, setTipoSolicitacao] = useState<'contratacao' | 'teste'>('contratacao');

  const handleContratarClick = (software: {nome: string, preco: string}) => {
    setSelectedSoftware(software);
    setTipoSolicitacao('contratacao');
    setFormOpen(true);
  };

  const handleTesteClick = (software: {nome: string, preco: string}) => {
    setSelectedSoftware(software);
    setTipoSolicitacao('teste');
    setFormOpen(true);
  };
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-3">
                      <Button 
                        ...
                        onClick={() => handleContratarClick(software)}
                      >
                        Contratar Agora
                      </Button>
                      <Button 
                        variant="outline" 
                        className="w-full"
                        onClick={() => handleTesteClick(software)}
                      >
